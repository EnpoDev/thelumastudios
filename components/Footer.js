...
import { Mail, MapPin, Phone, User, ArrowUpRight, Globe } from 'lucide-react';
...
export default function Footer({ locale = 'en' }) {
  ...
  return (
    <footer className="relative bg-cyber-darker text-white py-16 overflow-hidden">
      ...
      {/* Background glow */}
      <div className="absolute bottom-0 left-1/4 w-96 h-96 bg-neon-cyan/5 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute top-1/2 right-1/4 w-80 h-80 bg-neon-pink/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          <nav className="flex flex-wrap gap-1">
            {text.sections.map((section, index) => (
              <motion.a
                ...
                className="group relative px-4 py-2 text-gray-400 hover:text-white text-sm transition-colors"
                whileHover={{ y: -2 }}
              >
        ...
        <div className="border-t border-white/5 pt-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg hover:bg-white/5 transition-colors">
              <span className="text-neon-cyan mt-0.5 flex-shrink-0 text-xs font-bold">VKN</span>
              <span className="text-gray-400">{legalContact.vkn}</span>
            </div>
            <div className="flex items-start gap-3 p-3 rounded-lg hover:bg-white/5 transition-colors group">
              <span className="text-neon-pink mt-0.5 flex-shrink-0 text-xs font-bold">KEP</span>
              <a href={`mailto:${legalContact.kep}`} className="text-gray-400 group-hover:text-neon-pink transition-colors">
                {legalContact.kep}
              </a>
            </div>
          </div>
          <p className="text-gray-600 text-xs mt-4 italic pl-3 border-l-2 border-neon-cyan/30">
        ...
        <div className="border-t border-white/5 pt-8 mb-8">
          <div className="flex flex-wrap justify-center gap-2">
            <Link
              ...
              className="px-4 py-2 text-gray-400 hover:text-neon-cyan text-sm transition-all hover:bg-neon-cyan/5 rounded-lg"
            >
            ...
            <Link
              ...
              className="px-4 py-2 text-gray-400 hover:text-neon-pink text-sm transition-all hover:bg-neon-pink/5 rounded-lg"
            >
            ...
            <Link
              ...
              className="px-4 py-2 text-gray-400 hover:text-neon-purple text-sm transition-all hover:bg-neon-purple/5 rounded-lg"
            >
        ...
        <div className="border-t border-white/5 pt-8 mb-8">
          <div className="flex flex-wrap justify-center items-center gap-6">
            {/* Visa Logo */}
            <motion.div whileHover={{ scale: 1.05, filter: 'drop-shadow(0 0 10px rgba(0, 255, 255, 0.3))' }} className="opacity-50 hover:opacity-100 transition-opacity">
            ...
            {/* MasterCard Logo */}
            <motion.div whileHover={{ scale: 1.05, filter: 'drop-shadow(0 0 10px rgba(255, 0, 255, 0.3))' }} className="opacity-50 hover:opacity-100 transition-opacity">
              <svg className="h-8 w-auto" viewBox="0 0 750 471" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* iyzico Logo */}
            <motion.div whileHover={{ scale: 1.05, filter: 'drop-shadow(0 0 10px rgba(0, 255, 136, 0.3))' }} className="opacity-50 hover:opacity-100 transition-opacity">
              <Image
        ...
        <div className="border-t border-white/5 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <p className="text-gray-600 text-sm">
              © {currentYear} <span className="text-gray-400">Luma Studios</span>. {text.rights}.
            </p>
            ...
            <div className="flex items-center gap-4">
              <motion.a
                ...
                className="flex items-center gap-2 px-3 py-1.5 text-gray-400 hover:text-neon-cyan text-sm transition-colors rounded-lg hover:bg-neon-cyan/5"
                whileHover={{ scale: 1.02 }}
              >
              ...
              <motion.button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-gray-400 hover:text-neon-pink text-sm transition-colors rounded-lg hover:bg-neon-pink/5"
                whileHover={{ scale: 1.02 }}
              >
