...
import { FolderKanban, Upload, X, Plus, Edit2, Trash2, Star } from 'lucide-react';
...
export default function AdminProjects() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = editing ? `/api/projects?id=${editing}` : '/api/projects';
        ...
        <table className="min-w-full divide-y divide-gray-700/50">
          <thead className="bg-gray-900/80">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-300 uppercase tracking-wider">Slug</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-300 uppercase tracking-wider">Featured</th>
          ...
          <tbody className="divide-y divide-gray-700/30">
            {projects.map((project) => (
              <tr key={project.id} className="hover:bg-gray-800/30 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{project.title_en}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <button onClick={() => editProject(project)} className="inline-flex items-center text-purple-400 hover:text-purple-300 mr-4 font-semibold transition-colors">
                    <Edit2 className="w-4 h-4 mr-1" />Edit
                  </button>
                  <button onClick={() => handleDelete(project.id)} className="inline-flex items-center text-red-400 hover:text-red-300 font-semibold transition-colors">
